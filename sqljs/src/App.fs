...
let initBackend: obj -> unit = jsNative

module Url =
    type URLType =
        [<Emit("new $0($1,import.meta.url)")>]
        abstract CreateImportMetaUrl : url: string -> Browser.Types.URL
...
module CustomWorker =
    type CustomWorkerConstructor =
        [<Emit("new $0($1...)")>]
        abstract Create : url: Browser.Types.URL * ?options: Browser.Types.WorkerOptions -> Browser.Types.Worker

    [<Global>]
    let Worker: CustomWorkerConstructor = jsNative


let worker =
    CustomWorker.Worker.Create(Url.URL.CreateImportMetaUrl("./Worker.fs.js"))

initBackend (worker)

let api: Comlink.Remote<Worker.IApi> =
    Comlink.wrap (worker :?> Comlink.Protocol.Endpoint)
...
let init () =
    promise {
        ...
        let data =
            [| 1 .. 100 |]
            |> Array.map (fun i -> (JS.Math.random () * 1000., i))
        ...
        let mapped =
            users
            |> Seq.tryHead
            |> Option.map (UsersTable.mapUsers)
